...
// heat: optional setting to unlock wallet for block minting only;
...
// heat: total coins staked (non-spendable until maturity)
    ...
    BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
    {
      ...
      for(unsigned int i = 0; i < pcoin->vout.size(); i++)
      {
        ...
        if(pcoin->nTime > nSpendTime)
          continue;  // heat: timestamp must not exceed spend time
...

  {
    ...
    CTxDB txdb("r");
    {
      ...
      for(;;)
      {
        ...
        // heat: sub-cent change is moved to fee
        ...
        if(nChange > 0)
        {
          ...
          if(!GetBoolArg("-avatar")) // heat: not avatar mode
          {
...
// heat: create coin stake transaction
...
// heat: check 'spent' consistency between wallet and txindex
// heat: fix wallet spent state according to txindex
...
// heat: disable transaction (only for coinstake)
