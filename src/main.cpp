...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
  if(!fConnect)
  {
    // heat: wallets need to refund inputs when disconnecting coinstake
    if(tx.IsCoinStake())
    {
      BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
        if(pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
  ...
  for(int i = 0; i < vout.size(); i++)
  {
    ...
    // heat: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
  ...
  // heat: coinstake is also only valid in a block, not as a loose transaction
  if(tx.IsCoinStake())
...
// heat: find block wanted by given orphan block
...
int64 GetProofOfWorkReward(u32int nBits, bool fProofOfBurn)
{
  ...
  // heat: subsidy is cut in half every 16x multiply of difficulty
...
// heat: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// heat: find last block index up to pindex
...
static u32int GetNextTargetRequired(const CBlockIndex *pindexLast, bool fProofOfStake)
{
  ...
  // heat: target change every block
  // heat: retarget with exponential moving toward target spacing
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
  ...
  // heat: should not enter safe mode for longer invalid chain
}
                                 ...
                                 bool fMiner, bool fStrictPayToScriptHash)
{
  ...
  if(!IsCoinBase())
  {
    ...
    for(unsigned int i = 0; i < vin.size(); i++)
    {
      ...
      // heat: check transaction timestamp
    ...
    if(IsCoinStake())
    {
      // heat: coin stake tx earns reward instead of paying fee
      uint64 nCoinAge;
    ...
    else
    {
      ...
      // heat: enforce transaction fees for every block
      if(nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
  ...
  // heat: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
  ...
  // heat: track money supply and mint amount info
  ...
  // heat: fees are not collected by miners as in bitcoin
  // heat: fees are destroyed to compensate the entire network
  if(fDebug && GetBoolArg("-printcreation"))
...
// heat: total coin age spent in transaction, in the unit of coin-days.
...
// heat: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
  ...
  // heat: compute chain trust score
  pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

  // heat: compute stake entropy bit for stake modifier
  if(!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
    return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

  // heat: record proof-of-stake hash value
...
bool CBlock::CheckBlock() const
{
  ...
  // heat: only the second transaction can be the optional coinstake
  for(int i = 2; i < vtx.size(); i++)
    if(vtx[i].IsCoinStake())
      return DoS(100, error("CheckBlock() : coinstake in wrong position"));

  // heat: coinbase output should be empty if proof-of-stake block
...
bool CBlock::AcceptBlock()
{
  ...
  // heat: check that the block satisfies synchronized checkpoint
  ...
  // heat: check pending sync-checkpoint
...
bool ProcessBlock(CNode *pfrom, CBlock *pblock)
{
  ...
  // heat: verify hash target and signature of coinstake tx
  ...
  // heat: ask for pending sync-checkpoint if any
  ...
  if(!mapBlockIndex.count(pblock->hashPrevBlock))
  {
    ...
    if(pfrom)
    {
      ...
      // heat: getblocks may not obtain the ancestor block rejected
  ...
  // heat: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
  ...
  if(nFreeBytesAvailable < (uint64)15000000 + nAdditionalBytes)
  {
    ...
    printf("*** %s\n", strMessage.c_str());
    ThreadSafeMessageBox(strMessage, "heat", wxOK | wxICON_EXCLAMATION | wxMODAL);
    StartShutdown();
    return false;
  }
  return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
  ...
  printf("%s Network: \n\tgenesis=0x%s \n\tnBitsLimit=0x%08x \n\tnBitsInitial=0x%08x\n", 
         fTestNet ? "Test" : "heat", hashGenesisBlock.ToString().substr(0, 20).c_str(),
...
//////////////////////////////////////////////////////////////////////////////// 
    ...
    // heat: initialize synchronized checkpoint
    if(!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
      return error("LoadBlockIndex() : failed to init sync checkpoint");
  }

  // heat: if checkpoint master key changed must reset sync-checkpoint
  {
    CTxDB txdb;
    string strPubKey = "";
...
string GetWarnings(string strFor)
{
  ...
  // heat: wallet lock warning for minting
  if(strMintWarning != "")
  {
    nPriority = 0;
  ...
  // heat: should not enter safe mode for longer invalid chain
  // heat: if sync-checkpoint is too old do not enter safe mode
  ...
  // heat: if detected invalid checkpoint enter safe mode
  ...
  // Alerts
  {
    ...
    BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
    {
      ...
      if(alert.AppliesToMe() && alert.nPriority > nPriority)
      {
        ...
        if(nPriority > 1000)
          strRPC = strStatusBar;  // heat: safe mode for high alert
      }
    }
  }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
  ...
  if(strCommand == "version")
  {
    ...
    // heat: record my external IP reported by peer
      ...
      if(inv.type == MSG_BLOCK)
      {
        ...
        if(mi != mapBlockIndex.end())
        {
          ...
          if(inv.hash == pfrom->hashContinue)
          {
            ...
            // heat: send latest proof-of-work block to allow the
  ...
  else if(strCommand == "getblocks")
  {
    ...
    for(; pindex; pindex = pindex->pnext)
    {
      if(pindex->GetBlockHash() == hashStop)
      {
        ...
        // heat: tell downloading node about the latest block if it's
...
{
  /*heat: a burn hash is calculated by:
...
CBlock *CreateNewBlock(CWallet* pwallet, bool fProofOfStake, const CWalletTx *burnWalletTx, CReserveKey *resKey)
{
  ...
  // heat: if coinstake available add coinstake tx
  ...
  int64 nFees = 0;
  {
    ...
    while(!mapPriority.empty())
    {
      ...
      // heat: simplify transaction fee - allow free = false
...
bool CheckWork(CBlock *pblock, CWallet &wallet, CReserveKey &reservekey)
{
  ...
  if(pblock->IsProofOfWork() && hash > hashTarget)
    return error("heatMiner : proof-of-work not meeting target");

  if(pblock->IsProofOfBurn() && burnHash > hashBurnTarget)
    return error("heatMiner : proof-of-burn not meeting target");
  ...
  // Found a solution
  {
    ...
    if(pblock->hashPrevBlock != hashBestChain)
      return error("heatMiner : generated block is stale");
    ...
    if(!ProcessBlock(NULL, pblock))
      return error("heatMiner : ProcessBlock, block not accepted");
  }

  return true;
}

void static ThreadheatMiner(void* parg);

static bool fGenerateheats = false;
...
void heatMiner(CWallet *pwallet, bool fProofOfStake)
{
  ...
  while(fGenerateheats || fProofOfStake)
  {
    if(fShutdown)
      return;
    ...
    while(vNodes.empty() || IsInitialBlockDownload())
    {
      ...
      if(fShutdown)
        return;
      if((!fGenerateheats) && !fProofOfStake)
        return;
    }
    ...
    if(fProofOfStake)
    {
      // heat: if proof-of-stake block found then process block
      if(pblock->IsProofOfStake())
      {
    ...
    printf("Running heatMiner with %d %s in block\n", 
...
void static ThreadheatMiner(void* parg)
{
  ...
  try
  {
    ...
    heatMiner(pwallet, false);
    vnThreadsRunning[THREAD_MINER]--;
  }catch(std::exception& e)
  {
    vnThreadsRunning[THREAD_MINER]--;
    PrintException(&e, "ThreadheatMiner()");
  }catch(...)
  {
    vnThreadsRunning[THREAD_MINER]--;
    PrintException(NULL, "ThreadheatMiner()");
  }
  ...
  printf("ThreadheatMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}

void Generateheats(bool fGenerate, CWallet* pwallet)
{
  fGenerateheats = fGenerate;
  nLimitProcessors = GetArg("-genproclimit", -1);

  if(!nLimitProcessors)
    fGenerateheats = false;
  ...
  if(fGenerate)
  {
    ...
    printf("Starting %d heatMiner threads\n", nAddThreads);

    for(int i = 0; i < nAddThreads; i++)
    {
      if(!CreateThread(ThreadheatMiner, pwallet))
        printf("Error: CreateThread(ThreadheatMiner) failed\n");
