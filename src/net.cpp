...
{
  // Wait for IRC to get it first - disabled with heat
...
// testnet dns seed begins with 't', all else are heat dns seeds.
static const char *strDNSSeed[][2] = {
  {"seed", "dnsseed.heatpool.com"},
...
void ThreadDNSAddressSeed2(void* parg)
{
  ...
  if(true /*!fTestNet*/)  // heat enables dns seeding with testnet too
  {
...
// heat: burn miner after coins were burnt
...
void StartNode(void* parg)
{
  ...
  Generateheats(GetBoolArg("-gen", false), pwalletMain);

  // heat: mint proof-of-stake blocks in the background
  if(!CreateThread(ThreadStakeMinter, pwalletMain))
    printf("Error: CreateThread(ThreadStakeMinter) failed\n");

  // heat: hash proof-of-burn in the background
...
bool StopNode()
{
  ...
  if(vnThreadsRunning[THREAD_MINER] > 0)             printf("ThreadheatMiner still running\n");
